...
<table class="data-table" id="table">
  ...
  <tbody>
    ...
    //Run the correct DB query based on the order chosen by the the user
      ...
      } elseif ($orderby=="interest") {
        //This query counts the interest numbers of the events using the event interest page and then sorts by how popular it is
            ...
            <td>
              ...
              try{
                $stmt = $db->query("SELECT COUNT(*) AS interest, E.`id` FROM `interest_in_event` EI
                  ...
                  WHERE E.`id` = '$e_id'
                  GROUP BY E.`id`");
            //Works out how popular an event is based on interest for each event 
              ...
              if (isset($interest)){ ?>
                <a href='eventinterest.php?event_id=<?php echo $eventid ?>'> <?php echo $interest; ?></a>
              <?php 
            //The interent - given as a number then becomes a link (href) to the eventinterest.php page. This then allows the user to see who is signed up for their event 
            } else {
                echo "0";
                //print 0 if no interest
              }
